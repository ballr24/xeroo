...
function xeroomDownloadExportOrders( $xeroomOrderArray, $xeroomPaymentStatus, $xeroomRepeatStatus ) {
	...
	$xeroomUserName = '';
	foreach ($xeroomOrderArray as $i => $singleXeroomOrderArray) {
		$zeroom_r       = $i + 2;
		$zeroom_s       = $i + 1;
		$xeroomUserData = get_userdata( get_post_meta( $singleXeroomOrderArray, '_customer_user', true ) );
		if ( $xeroomUserData ) {
		...
		$objPHPExcel->setActiveSheetIndex( 0 )
		            ...
		            ->setCellValue( "C" . $zeroom_r, $singleXeroomOrderArray )
...
function xeroomDownloadExportLogFile( $xeroomOrderArray ) {
	...
	foreach ($xeroomOrderArray as $i => $singleXeroomOrderArray) {
		$zeroom_r       = $i + 2;
		$zeroom_s       = $i + 1;
		$xeroomUserData = get_userdata( get_post_meta( $singleXeroomOrderArray->order_id, '_customer_user', true ) );
		if ( $xeroomUserData ) {
			$xeroomUserName = $xeroomUserData->data->display_name;
		}
		$objPHPExcel->setActiveSheetIndex( 0 )
		            ->setCellValue( "A" . $zeroom_r, $zeroom_s )
		            ->setCellValue( "B" . $zeroom_r, $xeroomUserName )
		            ->setCellValue( "C" . $zeroom_r, $singleXeroomOrderArray->order_id )
		            ->setCellValue( "D" . $zeroom_r, $singleXeroomOrderArray->payment_type )
		            ->setCellValue( "E" . $zeroom_r, $singleXeroomOrderArray->ignore_type )
		            ->setCellValue( "F" . $zeroom_r, $singleXeroomOrderArray->status );
	}
...
class XLSXWorksheet {
	...
	protected function parseCellValue( $cell ) {
		switch ( (string) $cell["t"] ) {
			case "s":
				$value = (string) $cell->v != '' ? $this->workbook->sharedStrings[ (int) $cell->v ] : '';
				break;
			case "b":
				$value = (string) $cell->v;
				if ($value == '0') {
			...
			case "inlineStr":
				...
				break;
			case "e":
				$value = (string) $cell->v != '' ? (string) $cell->v : '';
				break;
			default:
				if ( ! (property_exists($cell, 'v') && $cell->v !== null) ) {
					return null;
				}
				$value = (string) $cell->v;
				if ( is_numeric( $value ) ) {
