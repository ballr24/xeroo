...
if ( ! empty( $woo_products ) ) {
	...
	foreach ( $woo_products as $woo_product ) {
		...
		if ( isset( $xero_line_items[ $product_sku ]['QuantityOnHand'] ) ) {
			$xero_stock = $xero_line_items[ $product_sku ]['QuantityOnHand'];
		}

		$xero_stock = '';
		if ( ! empty( $xero_line_items[ $product_sku ]['QuantityOnHand'] ) && $xero_line_items[ $product_sku ]['QuantityOnHand'] >= 0 ) {
...
if ( ! empty( $line_items ) ) {
	foreach ( $line_items as $type => $items_stock ) {
		if ( 'decrease' == $type ) {
			foreach ( $items_stock as $the_item ) {
